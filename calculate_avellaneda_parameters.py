...
def calculate_final_quotes(gamma, sigma, A, k, H, mid_price_df, ma_window):
    ...
    q = 0.001  # Placeholder for current inventory
...
def main():
    ...
    default_if_not_env = script_dir / 'GATE_data_collector' / 'gateio_data'
    print(default_if_not_env)
    HL_DATA_DIR = os.getenv('GATE_DATA_LOC', default_if_not_env)
    csv_file_path = os.path.join(HL_DATA_DIR, f'prices_{TICKER}_USDT.csv')
    ...
    trades_df = load_trades_data(os.path.join(HL_DATA_DIR, f'trades_{TICKER}_USDT.csv'))
